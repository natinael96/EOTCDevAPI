 ...
 */
import { jdnToGregorian } from './ethiopic.ts';
...
export function buildIcs(year: number, type: 'fasting' | 'feasts' | 'all'): string {
  ...
  const label = { fasting: 'Fasts', feasts: 'Feasts', all: 'Fasts & Feasts' }[type];
